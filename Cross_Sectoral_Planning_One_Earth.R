...
df_EBSA <- PUs %>% dplyr::mutate(Legend = case_when((NWIO_14 == TRUE) ~ "1",
                                                    (NEIO_09 == TRUE) ~ "2",
                                                    (SIO_11 == TRUE) ~  "3",
                                                    (SIO_19 == TRUE) ~ "4",
                                                    (SIO_22 == TRUE) ~ "5",
                                                    (SIO_32 == TRUE) ~ "6",
                                                    (SIO_23 == TRUE & SIO_11 == TRUE) ~ "7",
                                                    (SIO_35 == TRUE) ~ "8",
                                                    (SIO_36 == TRUE) ~ "9",
                                                    (SIO_37 == TRUE) ~ "10",
...
EBSA_plot <- ggplot() + 
  ...
  scale_fill_manual(values = c("1" = "dodgerblue4", 
                               "2" = "dodgerblue1",
                               "3" = "deepskyblue1",
                               "4" = "lightskyblue2",
                               "5" = "paleturquoise",
...
df_contracts <- PUs %>% dplyr::mutate(Legend = case_when((ExplPMN == TRUE) ~ "Nodules (exp.)",
                                                         (ResPMN == TRUE) ~ "Nodules (res.)",
                                                         (ExplPMS == TRUE) ~ "Sulphides (exp.)",
...
Contracts_plot <- ggplot() + 
  ...
  scale_fill_manual(values = c("Nodules (exp.)" = "goldenrod2",
                               "Nodules (res.)" = "goldenrod3",
                               "Sulphides (exp.)" = "red2")) +
  geom_sf(data = world, fill = "grey20", size = 0.05, show.legend = FALSE) +
  coord_sf(xlim = c(st_bbox(Bndry)$xmin, st_bbox(Bndry)$xmax), 
           ylim = c(st_bbox(Bndry)$ymin, st_bbox(Bndry)$ymax),
...
myPaletteRC <- colorRampPalette(rev(brewer.pal(11, "RdYlBu"))) #RdYlBu
...
fishing_plot <- ggplot()+
  ...
  coord_sf(xlim = c(st_bbox(Bndry)$xmin, st_bbox(Bndry)$xmax), 
           ...
           expand = TRUE) +
  theme_bw() +
  ggtitle("Opportunity cost: fishing") +
  theme(axis.text.x = element_text(size = 10),
        axis.text.y = element_text(size = 10),
        legend.title = element_text(size = 11),
        legend.text = element_text(size = 10),
...
p_FishPlanA_Sol_Plot <- ggplot() +
  ...
  geom_sf(data = world, fill = "grey20", size = 0.05)  +
  geom_sf(data = p_FishPlanA_Sol, aes(fill = as.factor(solution_1)), size = 0.01) +
  scale_fill_manual(values = c("grey95", "blue"), labels = c("Not selected", "Selected")) +
  coord_sf(xlim = c(st_bbox(Bndry)$xmin, st_bbox(Bndry)$xmax), 
           ylim = c(st_bbox(Bndry)$ymin, st_bbox(Bndry)$ymax),
           expand = TRUE) +
  theme_bw() +
  theme(axis.text.x = element_text(size = 10),
        ...
        legend.text = element_text(size = 10),
        legend.key.width = unit(0.7,"cm")) +
  theme(panel.grid.major=element_line(colour="grey95", size = 0.2),
...
mining_plot <- ggplot()+
  ...
  coord_sf(xlim = c(st_bbox(Bndry)$xmin, st_bbox(Bndry)$xmax), 
           ...
           expand = TRUE) +
  theme_bw() +
  ggtitle("Opportunity cost: mining") +
  theme(axis.text.x = element_text(size = 10),
        axis.text.y = element_text(size = 10),
        legend.title = element_text(size = 11),
        legend.text = element_text(size = 10),
...
shipping_plot <- ggplot() +
  ...
  sc_ship +
  ggtitle("Opportunity cost: shipping") +
  geom_sf(data = world, size = 0.05, fill = "grey20") +
  coord_sf(xlim = c(st_bbox(Bndry)$xmin, st_bbox(Bndry)$xmax), 
           ylim = c(st_bbox(Bndry)$ymin, st_bbox(Bndry)$ymax),
           expand = TRUE) +
...
cbp_df_sliced <- dplyr::slice(cbp_df, -60, -43, -26, -9) %>% #remove NaN rows
  dplyr::mutate(individual = dplyr::case_when(stringr::str_detect(Features, pattern = "IBA") ~ "Important Bird Areas",
                                              str_detect(Features, pattern = "IMMA") ~ "Marine Mammal Areas",
                                              ...
                                              str_detect(Features, pattern = "SIO_11") ~ "Agulhas Front",
...
gg30 <- ggplot(cbp_df_sliced_30, aes(x=individual, y=Targets, fill = Scenario)) + 
  ...
  scale_x_discrete(labels = function(x) str_wrap(x, width = 2)) # to avoid labels going out of panel when too long #https://statisticsglobe.com/wrap-long-axis-labels-ggplot2-plot-into-multiple-lines-r


gg30 +
  scale_x_discrete(labels = function(x) str_wrap(x, width = 4))#https://statisticsglobe.com/wrap-long-axis-labels-ggplot2-plot-into-multiple-lines-r
...
base_plot_5 <- PUs_plot + Contracts_plot + IMMA_plot + IBA_plot + EBSA_plot + Vents_plot + Plateaus_plot + Seamounts_plot + fishing_plot + shipping_plot + mining_plot +
...
ggsave("pdfs/circbarplotsBIG.jpeg", width = 10, height = 12, dpi = 1200)
...
rotate_data <- function(data, x_add = 0, y_add = 0) {
  ...
  shear_matrix <- function(){ matrix(c(1.2, 0.2, 0, 1), 2, 2) }
  ...
  data %>% 
    dplyr::mutate(
      geometry = .$geometry * shear_matrix() * rotate_matrix(pi/90) + c(x_add, y_add)
    )
}
...
rotate_data <- function(data, x_add = 0, y_add = 0) {
  ...
  data %>% 
    dplyr::mutate(
      geometry = .$geometry * shear_matrix() * rotate_matrix(pi/1) + c(x_add, y_add)
    ) # change division of pi to change inclination of map
}
...
p_FishPlanA_Sol %>% 
  as_tibble() %>% 
  filter(solution_1 == 1) %>% 
  summarise(CostFishing = sum(Fishing), #372652 out of 1913029 = 0.1947968
            CostMining = sum(MiningCost), #1.90e12 out of 3.49e13 = 0.05444126
            CostShip = sum(ShippingIntensity)) # 15861714 out of 211501215 = 0.07499585

p_ShipPlanA_Sol %>% 
  as_tibble() %>% 
   filter(solution_1 == 1) %>% 
  summarise(CostFishing = sum(Fishing), #699768 out of 1913029 = 0.3657906
            CostMining = sum(MiningCost), #2298125225024 out of 3.49e13  = 0.06584886
            CostShip = sum(ShippingIntensity)) # 1313998 out of 211501215 = 0.006212721

p_MinePlanA_Sol %>% 
  as_tibble() %>% 
   filter(solution_1 == 1) %>% 
  summarise(CostFishing = sum(Fishing), #977467 out of 1913029 = 0.5109525
            CostMining = sum(MiningCost), #542316362224 out of 3.49e13 = 0.01553915
            CostShip = sum(ShippingIntensity)) # 53864360 out of 211501215 = 0.2546764

combined <- tibble::as_tibble(readRDS("combined.rds"))

### merge PUs with combined solutions to get cost of union of sector-specific plans. This gives the relative opportunity cost incurred by each sector if all sector-specific plans are implemented.


PUs_tibble <- tibble::as_tibble(PUs)

joined_all <- inner_join(PUs_tibble, combined)

joined_all %>% 
  as_tibble() %>% 
  filter(joined_all$mining_sol == 'Selected' | joined_all$shipping_sol == 'Selected' | joined_all$fishing_sol == 'Selected') %>% 
  summarise(UnionFishing = sum(FishingCost), #1023202 out of 1913029 = 0.5348596
            UnionMining = sum(MiningCost), #2.70e12 out of 3.49e13 = 0.0773639
            UnionShip = sum(ShippingIntensity)) # 55677510 out of 211501215 = 0.2632491

# find size of union

joined_all$union[joined_all$mining_sol == 'Selected' | joined_all$shipping_sol == 'Selected' | joined_all$fishing_sol == 'Selected'] <- 1
...
fSolnText <- function(sDat, col_name) {    # fx for getting % selected PUs
  ...
  percPU <- ceiling(PU_count/nrow(sDat) * 100) # changed round to ceiling
  return(percPU)}
...
combined <- mining %>%
  dplyr::mutate(fishing_sol = fishing$solution_1, 
                shipping_sol = shipping$solution_1) %>%
  dplyr::rename(mining_sol = solution_1) %>%
  st_as_sf()

Combined <- combined %>% 
  dplyr::mutate(solution = as.factor(case_when((fishing_sol == 1 & shipping_sol == 0 & mining_sol == 0) ~ 1,
                                               (fishing_sol == 0 & shipping_sol == 1 & mining_sol == 0)  ~ 2,
                                               (fishing_sol == 0 & shipping_sol == 0 & mining_sol == 1)  ~ 3,
                                               (fishing_sol == 1 & shipping_sol == 1 & mining_sol == 0)  ~ 4,
                                               (fishing_sol == 1 & shipping_sol == 0 & mining_sol == 1)  ~ 5,
                                               (fishing_sol == 0 & shipping_sol == 1 & mining_sol == 1)  ~ 6,
                                               (fishing_sol == 1 & shipping_sol == 1 & mining_sol == 1)  ~ 7,
                                               TRUE ~ 0)) )


perc_mining <- fSolnText(Combined, "mining_sol")
perc_fishing <- fSolnText(Combined, "fishing_sol")
perc_shipping <- fSolnText(Combined, "shipping_sol")
perc_cross <- fSolnText(cross, "solution_1")

FishingPlot3 <- ggplot() +
  ggtitle("Optimal MPAs for the fishing sector") +
  geom_sf(data = world, fill = "grey20", size = 0.05)  +
  geom_sf(data = p_FishPlanA_Sol, aes(fill = as.factor(solution_1)), size = 0.01) +
  scale_fill_manual(values = c("#c6dbef", "blue"), labels = c("Not selected", "Selected")) +
  coord_sf(xlim = c(st_bbox(Bndry)$xmin, st_bbox(Bndry)$xmax), 
           ylim = c(st_bbox(Bndry)$ymin, st_bbox(Bndry)$ymax),
           expand = TRUE) +
  theme_bw() +
...
txt_solnFishing <-  paste0('MPAs: 8%')

FishingPlotAnn <- FishingPlot3 + ggplot2::annotate(geom = "label", label = txt_solnFishing, x = Inf, y = Inf, hjust = 1.05, vjust = 1.5, size = 5)

ggsave(filename = "FishingPlotAnn.png",
       plot = FishingPlotAnn, width = 15, height = 15, dpi = 300,
       path = "pdfs/") # save plot, change path accordingly

ShipPlot3 <- ggplot() +
  ggtitle("Optimal MPAs for the shipping sector") +
  geom_sf(data = world, fill = "grey20", size = 0.05, show.legend = FALSE)+
  geom_sf(data = p_ShipPlanA_Sol, aes(fill = as.factor(solution_1)), size = 0.01) +
  scale_fill_manual(values = c("#c6dbef", "blue"), labels = c("Not selected", "Selected")) +
  coord_sf(xlim = c(st_bbox(Bndry)$xmin, st_bbox(Bndry)$xmax), 
...
txt_solnShipping <-  paste0('MPAs: 17%')

ShipPlotAnn <- ShipPlot3 + ggplot2::annotate(geom = "label", label = txt_solnShipping, x = Inf, y = Inf, hjust = 1.05, vjust = 1.5, size = 5)

ggsave(filename = "ShippingPlotAnn.png",
       plot = ShipPlotAnn, width = 15, height = 15, dpi = 300,
       path = "pdfs/") # save plot, change path accordingly

MinePlot3 <- ggplot() +
  ggtitle("Optimal MPAs for the deep-sea mining sector") +
  geom_sf(data = world, fill = "grey20", size = 0.05, show.legend = FALSE)+
  geom_sf(data = p_MinePlanA_Sol, aes(fill = as.factor(solution_1)), size = 0.01) +
  scale_fill_manual(values = c("#c6dbef", "blue"), labels = c("Not selected", "Selected")) +
  coord_sf(xlim = c(st_bbox(Bndry)$xmin, st_bbox(Bndry)$xmax), 
...
txt_solnMining <-  paste0('MPAs: 22%')

MinePlotAnn <- MinePlot3 + ggplot2::annotate(geom = "label", label = txt_solnMining, x = Inf, y = Inf, hjust = 1.05, vjust = 1.5, size = 5)

ggsave(filename = "MiningPlotAnn.png",
       plot = MinePlotAnn, width = 15, height = 15, dpi = 300,
       path = "pdfs/") # save plot, change path accordingly

CrossPlot3 <- ggplot() +
  ggtitle("Optimal MPAs for all sectors") +
  geom_sf(data = world, fill = "grey20", size = 0.05, show.legend = FALSE)+
  geom_sf(data = p_PlanFinal_Sol, aes(fill = as.factor(solution_1)), size = 0.01) +
  scale_fill_manual(values = c("#c6dbef", "blue"), labels = c("Not selected", "Selected")) +
  coord_sf(xlim = c(st_bbox(Bndry)$xmin, st_bbox(Bndry)$xmax), 
...
txt_solnCross <-  paste0('MPAs: 8%')

CrossPlotAnn <- CrossPlot3 + ggplot2::annotate(geom = "label", label = txt_solnCross, x = Inf, y = Inf, hjust = 1.05, vjust = 1.5, size = 5)

ggsave(filename = "CrossPlotAnn.png",
       plot = CrossPlotAnn, width = 15, height = 10, dpi = 1800,
       path = "pdfs/") # save plot, change path accordingly

#p_MinePlanA_Sol_Plot_ann <- p_MinePlanA_Sol_Plot + ggplot2::annotate(geom = "text", label = '16.9% of planning region selected in MPA', x = Inf, y = Inf, hjust = 1.05, vjust = 1.5, size = 4.5, colour = 'black')

ReplacementCross <- eval_replacement_importance(p_PlanFinal, dfCross, rescale = FALSE, force = TRUE) 

save.image("~/Documents/MscThesis/GitHub/SpatialPlanning/IndianOceanCrossRC.RData")

saveRDS(ReplacementCross, paste0("~/Documents/MscThesis/GitHub/SpatialPlanning/", "ReplacementCross.rds")) # save solution

# Defining palette
myPalette <- colorRampPalette(rev(brewer.pal(3, "Oranges"))) #RdYlBu
nam <- parse(text = "log[10] (cost)")
namrc <- parse(text = "log[10] (score)")
sc_cross <- scale_colour_gradientn(name = nam, colours = myPalette(10), na.value = "lightgrey", limits=c(0, 4), aesthetics = c("color","fill"))
sc_cross_rc <- scale_colour_gradientn(name = namrc, colours = myPalette(10), na.value = "lightblue", limits=c(0, 4), aesthetics = c("color","fill"))

rc_cross_plot <- ggplot() + 
  sc_cross_rc +
  ggtitle("Replacement cost score for cross-sectoral MPAs") +
  geom_sf(data = world, fill = "grey20", size = 0.05)+
  geom_sf(data = ReplacementCross, aes(color=log10(rc), fill = log10(rc))) + 
  sc_cross_rc +
  coord_sf(xlim = c(st_bbox(Bndry)$xmin, st_bbox(Bndry)$xmax), 
           ylim = c(st_bbox(Bndry)$ymin, st_bbox(Bndry)$ymax),
           expand = TRUE) +
...
AllPlots <- FishingPlotAnn + rc_fish_plot + ShipPlotAnn + rc_ship_plot + MinePlotAnn + rc_mine_plot + CrossPlotAnn + rc_cross_plot +
  plot_layout(nrow = 4) +
  plot_annotation(tag_levels = 'A', tag_suffix = ')') &
  theme(plot.tag = element_text(face = 'bold')) & 
  theme(legend.justification = "left")

ggsave("pdfs/AllPlots.jpeg", width = 15, height = 20, dpi = 300)
